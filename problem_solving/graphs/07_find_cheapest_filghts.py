...
def findCheapestPrice(n: int,
    ...
    Time complexity: O(VlogV + E), because we visit each node once and each edge once, and we use a priority queue
    Space complexity: O(V + E), because we store the graph as an adjacency list
    """
    graph = defaultdict(dict)
    for a, b, price in flights:
        graph[a][b] = price
    heap = [(0, src, k + 1)] # (price, city, stops)
    while heap:
        ...
        if stops > 0:
            for neighbor in graph[city]:
                new_price = price + graph[city][neighbor]
